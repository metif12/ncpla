<x-form-section submit="storeReport">
    ...
    <x-slot name="form">
        ...
        @foreach($outputs as $i => $output)
            ...
            <div class="col-start-1 col-span-6 sm:col-span-4">
                <x-label for="output.{{$i}}.product_id" value="{{ $i+1 }}. خروجی مربوط به ورودی"/>
                <select
                    class="form-input rounded-md shadow-sm mt-1 block w-full"
                    id="output.{{$i}}.product_id" wire:model.lazy="output.{{$i}}.product_id">
                    <option
                        value="0"
                    >هیچکدام</option>
                ...
                <x-input-error for="output.{{$i}}.product_id" class="mt-2"/>
            </div>

            <div class="col-start-1 col-span-6 sm:col-span-4">
                <x-label for="output.{{$i}}.code" value="شماره شناسایی"/>
                <x-input id="output.{{$i}}.code" type="text" step="any" class="mt-1 block w-full" wire:model.lazy="output.{{$i}}.code"/>
                <x-input-error for="output.{{$i}}.code" class="mt-2"/>
            </div>
